...
const FormularioRespuesta = ({ isOpen, onClose, onGuardar, tipoSolicitud = 'Respuesta a Oposición', form: propForm, setForm: propSetForm, errors: propErrors, setErrors: propSetErrors }) => {
  ...
  const [localForm, setLocalForm] = useState({
    ...
    marcaOpositora: '', // ✅ Nombre correcto según especificación
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setForm({
        ...
        marcaOpositora: '', // ✅ Nombre correcto
  ...
  const validate = (customForm) => {
    ...
    if (!f.marcaOpositora) e.marcaOpositora = 'Requerido';
    else if (!/^[A-Za-zÁÉÍÓÚáéíóúÑñ0-9 .,&-]{2,100}$/.test(f.marcaOpositora)) e.marcaOpositora = 'Solo letras, números y básicos, 2-100 caracteres';
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      console.log('🔧 [FormularioRespuesta] Formulario válido, llamando onGuardar');
